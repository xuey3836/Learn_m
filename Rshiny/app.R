...
server <- function(input, output) {
  output$hsy <- renderImage({
    imagefile <- normalizePath('huashengyou.jpg') ## 设置文件名
    list(src=imagefile)  ## 这是expr的返回值
  },deleteFile = FALSE)
  output$hsj <- renderImage({
    imagefile <- normalizePath('huashengjiang.jpg') ## 设置文件名
    list(src=imagefile)  ## 这是expr的返回值
  },deleteFile = FALSE)
